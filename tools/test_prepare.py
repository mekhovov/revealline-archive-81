...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1149, 595667178))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.114.0'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-81/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], 'ad9bd22e8240dfb11031796b88ca02896f37f0af')
        self.assertEqual(release['sourceRevision'], '10190539b243c9edc78eeef9dba174041409aaf2')
        self.assertEqual(release['sourceTree'], '026404692da6c4a03f2a23fbedbcf015c4df39f2')
        self.assertEqual(release['sourceQualification'], {'bytes': 14729, 'sha256': '9e9b4fe8c1bde82c535858fd2d052eed666192c0abcb8f367fdb1210ce3c8398', 'policyEvidence': {'path': 'publishing/test-policy.json', 'bytes': 490, 'sha256': 'b6887ba7f2b84a007b96de14fc867ac38ac8135d7c231c7ade7ebc52ba31704a'}})
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.114.0/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (1146, 595665781))
